...
> **If this is a spec task**: omit this section - `## Context Files` in `spec.md`
> already lists the domains relevant to every task in the spec.
>
> **If this is a one-off task** (no parent spec): list the domains relevant to
> this task, same format as `spec-template.md`.
...
*Claude-maintained - do not hand-edit.*

> **If this is a spec task**: omit this section - `## Required Context` in
> `spec.md` already covers every task in the spec.
>
> **If this is a one-off task** (no parent spec): list files outside the
> `## Context Files` domain-folder mechanism that this task must read before
> work proceeds - design guides, glossary entries, related specs, or
> anything else surfaced during authoring or a later human correction. Read
> unconditionally by `STARTING-ONE-OFF-TASK.md`, regardless of whether the
> task looks related to the listed file.
